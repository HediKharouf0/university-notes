...
\textbf{Def 2.1.1 (Prob. space):} $\Omega$: realisations, $P: \mathcal{P}(\Omega) \to [0,1]$:\newline
2. $A_i \cap A_j = \varnothing$ for $i \neq j \Rightarrow P(\bigcup_{i \in \mathbb{N}} A_i) = \sum_{i \in \mathbb{N}} P(A_i)$\newline
\textbf{Thm 2.1.1:} $P(\Omega \setminus A) = 1 - P(A)$; $A \subset B \Rightarrow P(A) \leq P(B)$\newline
$P(A \cup B) = P(A) + P(B) - P(A \cap B)$\newline
$A_i \cap A_j = \varnothing \Rightarrow P(\bigcup A_i) = \sum P(A_i)$ (finite/countable)\newline
$P(\bigcup A_i) \leq \sum P(A_i)$ ($\sigma$-sub-additivity)\newline
...
\textbf{Def 2.1.2 (pmf):} $\Omega$ finite/countable, $p: \Omega \to [0,1]$, $\sum_{\omega} p(\omega) = 1$\newline
$P_p(A) = \sum_{\omega \in A} p(\omega)$ (probab. measure)\newline
\textbf{Def 2.1.3 (pdf):} $f: \mathbb{R}^d \to [0,+\infty)$, $\int \cdots \int f(x_1,\ldots,x_d)dx = 1$\newline
...
$(A_i)_{i \in I}$ two-by-two indep.: $\forall i \neq j$, $A_i, A_j$ indep.\newline
...
$(X_i)_{i\in I}$ two-by-two indep.: $\forall i \neq j$, $X_i, X_j$ indep.\newline
...
\textbf{Lem 2.4.3 (Total prob.):} $(A_i)$ partition, $P(A_i) > 0$:\newline
...
\textbf{Thm 2.7.1 (Markov):} $X \geq 0 \Rightarrow P(X \geq a) \leq \frac{E(X)}{a}$, $\forall a > 0$\newline
\textbf{Lem 2.7.2:} $g$ increasing, $g(X)$ r.v.: $P(X \geq a) \leq \frac{E(g(X))}{g(a)}$\newline
...
\textbf{Thm 2.7.3 (wLLN):} $X_1,X_2,\ldots$ identically distributed, $E(X_1^2) < \infty$,\newline
...
\textbf{(Converg. in law):} $X,X_1,X_2,\ldots: \Omega \to \mathbb{R}$ r.v. $X_n \xrightarrow{\text{Law}} X$ \Leftrightarrow:\newline
...
\bullet $X,X_1,\ldots: \Omega \to \mathbb{R}$ r.v. Suppose $\exists \delta > 0$:
...
\bullet$X: \Omega \to \mathbb{R}$ r.v. with $\text{Image}(X) \subset \mathbb{Z}$, $X_1,X_2,\ldots: \Omega \to \mathbb{R}$ r.v.\newline
...
\bullet $X,X_1,X_2,\ldots: \Omega \to \mathbb{R}$ r.v. $X_n \xrightarrow{\text{Proba}} X$ \Leftrightarrow\: $\forall \epsilon > 0$:
$\lim_{n\to\infty} P(|X - X_n| \geq \epsilon) = 0$\newline
\bullet $X,X_1,\ldots: \Omega \to \mathbb{R}$ r.v. $X_n \xrightarrow{a.s.} X$ \Leftrightarrow\:
...
\textbf{(Converg. in $L^p$):} $X_n \xrightarrow{L^p} X$:\:lim_{n \to \infty} $$E[|X_n-X|^p] = 0$\newline
\textbf{Converg. hierarchy:} $L^\infty \Rightarrow$ a.s. $\Rightarrow$ Proba $\Rightarrow$ Law;\newline $L^\infty \Rightarrow$ $L^p \Rightarrow$ Proba $\Rightarrow$ Law\newline
...
where $\mu = E(X)$, $\sigma_X^2 = \text{Var}(X)$\newline
...
$X,X_1,\ldots$ indep. fam. of id. distr. r.v. If $\exists \delta > 0$: $E(e^{\delta|X|}) < \infty$:\newline
$\frac{1}{\sqrt{\sigma_X^2 n}}\sum_{i=1}^n (X_i - \mu) \xrightarrow{\text{Law}} \mathcal{N}(0,1)$ as $n \to \infty$ ( $\mu = E(X)$, ...)
\textbf{Lem 3.6.1:} $k \geq 2$, $p_1,\ldots,p_k \in (0,1)$: $p_1 + \cdots + p_k = 1$. For $n \geq 1$,\newline
$(N_{n,1},\ldots,N_{n,k})$ multinomial r.v. with parameters $(n,k;p_1,\ldots,p_k)$. Then:\:
$\sum_{i=1}^k \frac{(N_{n,i} - np_i)^2}{p_i} \xrightarrow{\text{Law}} \chi_{k-1}^2$ as $n \to \infty$
...
$(\hat{f_n})_{n \geq 1}$ is \textit{convergent} (or \textit{consistent}) if $\hat{f_n}$ \xrightarrow{\text{Proba}} \: $f(\theta)$:\newline
...
\bullet $ \text{Var}_\theta(\hat{f}_n) = \text{Var}(\hat{f}_n(X_1,\ldots,X_n))$\newline
...
\Rightarrow \hat{f}_n$ is a convergent sequence of estimators
...
\textbf{Def 4.4.1 (CI):} $X_1,\ldots,X_n$ sample of law $\mathbb{P}_\theta$. $\alpha \in (0,1)$. Random interval $I = I(X_1,\ldots,X_n)$ not depending on $\theta$ is level $1-\alpha$ confidence interval for $f(\theta)$ if $\forall \theta \in \Theta$:\:
$P(f(\theta) \in I(X_1,\ldots,X_n)) = 1-\alpha$\newline
\textbf{Def 4.4.2 (Excess CI):} CI $I = I(X_1,\ldots,X_n)$ is excess confidence interval for $f(\theta)$ at level $1-\alpha$ if\:
...
\textbf{Def 4.5.2 (Rejection region):} $D$ event for r.v. $X_1,\ldots,X_n$. I.e.: if $X_i$ take values in $\mathbb{R}^d$, $D \subset (\mathbb{R}^d)^n$. In practice:\newline
$D = \{(x_1,\ldots,x_n): T(x_1,\ldots,x_n) \in [a,b]\}$ for some statistic $T$ and $a < b$\newline
\textbf{Def 4.5.3 (Test procedure):} Given $D$ rejection region, $H_0,H_1$ hypotheses:\newline
...
\textbf{Def 4.5.4 (Risk/Conf. level):} $\alpha \in [0,1]$. Test has risk level $\alpha$ or confidence level $1-\alpha$ if\newline
$\sup_{\theta \in \Theta_0} P((X_1,\ldots,X_n) \in D) = \alpha$\newline
\textbf{Def 4.5.5 (Power):} Power of test:\newline
...
$X \sim \text{Student}_t(\nu)$: $f_X(x) = \frac{\Gamma(\frac{\nu+1}{2})}{\sqrt{\pi\nu}\Gamma(\frac{\nu}{2})}\left(1+\frac{x^2}{\nu}\right)^{-\frac{\nu+1}{2}}$\newline
$Z \sim \mathcal{N}(0,1)$, $V \sim \chi_k^2$ indep. $\Rightarrow Z\sqrt{\frac{k}{V}} \sim \text{Student}_t(k)$\newline
\textbf{One-sample:} $X_i \sim \mathcal{N}(\mu,\sigma^2)$, test $H_0$: $\mu = \mu_0$\newline
$T = \frac{\sqrt{n}(\bar{X}_n - \mu_0)}{s_X} \sim \text{Student}_t(n-1)$ under $H_0$, $s_X^2 = \frac{1}{n-1}\sum(X_i - \bar{X}_n)^2$\newline
\textbf{Two-sample:} $X_i \sim \mathcal{N}(\mu_X,\sigma^2)$, $Y_i \sim \mathcal{N}(\mu_Y,\sigma^2)$ indep., test $H_0$: $\mu_X = \mu_Y$\newline
$T_n = \frac{\sqrt{n}(\bar{X}_n - \bar{Y}_n)}{\sqrt{s_X^2 + s_Y^2}} \sim \text{Student}_t(2n-2)$ under $H_0$\newline
Rejection: $\{|T| \geq C\}$, $C$ from confidence level
...
$\frac{S_X^2}{S_Y^2} \stackrel{\text{Law}}{=} \frac{\sigma_X^2}{\sigma_Y^2} \cdot Z$, $Z \sim \text{F}(n-1,m-1)$. Under $H_0$ ($\sigma_X^2 = \sigma_Y^2$): $\frac{S_X^2}{S_Y^2} \sim \text{F}(n-1,m-1)$\newline
Rejection: $D = \left\{\frac{S_X^2}{S_Y^2} \notin [1-c_\alpha^-,1+c_\alpha^+]\right\}$, $c_\alpha^\pm$ from $\text{F}(n-1,m-1)$ quantiles
...
\textbf{Def 4.6.1 (MSE):} $n \geq 1$, $X_1,\ldots,X_n$ $n$-sample of law $\mathbb{P}_\theta$, $\hat{f}$ estimator of $f(\theta)$. Mean square error:\newline
$\text{MSE}_\theta(\hat{f}) = E((\hat{f}(X_1,\ldots,X_n) - f(\theta))^2)$\newline

\textbf{Def 4.6.2 (MMSE):} $n \geq 1$, $X_1,\ldots,X_n$ $n$-sample of law $\mathbb{P}_\theta$, $f(\theta)$ quantity to estimate. Minimal mean square error estimator of $f(\theta)$:\newline
$\text{MMSE}_{f(\theta)}(X_1,\ldots,X_n) = \arg\min_{\hat{f}} E((\hat{f}(X_1,\ldots,X_n) - f(\theta))^2)$\newline
where minimum over all estimators $\hat{f}(X_1,\ldots,X_n)$ of $f(\theta)$. Existence non-trivial, generally not easy to compute\newline

\subsubsection*{4.6.2 Asymptotic normality}
\textbf{Def 4.6.3 (Asymp. normal):} $n \geq 1$, $X_1,\ldots,X_n$ $n$-sample of law $\mathbb{P}_\theta$, $\hat{f}_n$ estimator of $f(\theta)$. $\hat{f}_n$ is asymptotically normal sequence of estimators if:\newline
